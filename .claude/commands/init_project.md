...
| @src/server/routes.ts | Express backend — SSE streaming, all API endpoints, `GET /api/config` (includes `gemini_api_key`), `express.static()` for production. cwd-scoped: `PROJECT_CWD` from launch dir |
...
- **Gemini API key flow**: `GEMINI_API_KEY` in `.env` → Express reads it via `loadEnv()` in `cli.ts` → served at runtime via `GET /api/config` (`gemini_api_key` field) → frontend fetches on mount in `+page.svelte` (`$state`, not const). Fallback to `import.meta.env.VITE_GEMINI_API_KEY` for dev (Vite bakes `VITE_*` vars at build time). **Critical for publishing**: never use `VITE_` prefix for secrets — Vite bakes them into the JS bundle, which ships to npm. The `envPrefix` in `vite.config.ts` is `'VITE_'` only (removed `'GOOGLE_'` to prevent accidental key leaks). Backend also accepts `VITE_GEMINI_API_KEY` as fallback in `/api/config` for backward compat.
