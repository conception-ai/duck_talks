...
| @src/server/routes.ts | Express backend — SSE streaming, all API endpoints, `GET /api/config` (serves `gemini_api_key`), `express.static()` for production. cwd-scoped: `PROJECT_CWD` from launch dir |
| @src/server/cli.ts | CLI entry point. `.env` loader (zero-dep), prereq checks (`GEMINI_API_KEY` required), Express listen, `publicDir` resolution for production mode |
...
| @src/client/routes/live/+page.svelte | Single-page app — sidebar (collapsed by default, 52px→226px) + main content. Sidebar: Reduck logo, "New session" link, recent sessions list from `GET /api/sessions`. Main: dark theme (`reduck-theme`). Unified `.input-box` morphs via `InputMode` (`idle`\|`recording`\|`review`\|`streaming`). Idle: textarea + model select + mic btn (voice) or send btn (typed). Recording: readonly textarea + waveform + stop X. Review: editable textarea + Accept/Reject. Streaming: readonly + stop. Settings popover (gear). Two send paths: `live.start()` (voice) and `live.sendText()` (typed). Waveform via second `getUserMedia` + `AnalyserNode`. Loads history on mount. Renders `messages[]` only |
...
- **Gemini API key flow**: Server env var `GEMINI_API_KEY` (required, crashes if missing) → served via `GET /api/config` → fetched on page mount into `$state<string | null>` → passed to data store via `getApiKey: () => apiKey` closure. The closure captures the reactive `$state` reference (Svelte 5 runes), not a snapshot — so it returns the current value even though the fetch is async. No more localStorage for the key.
