...
- **Smart interrupt model** (`gemini.ts`): `inputTranscription` does NOT abort `activeConverse`. Two separate abort paths:
  - **Keyword fast-path**: `/\bstop\b/i` on transcript → immediate `activeConverse.abort()` (~0ms). False positives on "bus stop" etc. are accepted.
  - **Tool-based**: Gemini processes speech, calls `stop` tool → abort via tool handler (~1-2s latency). Or calls `converse` with new instruction → `executeConverse` calls `activeConverse?.abort()` at line 120 before starting new stream.
  - **`sc.interrupted` still aborts**: This is Gemini's own VAD interruption signal (model generation was canceled server-side), distinct from `inputTranscription` (ASR text). When `sc.interrupted` fires, `activeConverse?.abort()` runs. This is correct — the model itself is restarting.
  - **Nudge may over-trigger**: `turnComplete` without tool call after `userSpokeInTurn=true` sends "You did not call the converse tool. Please call it now." With the smart interrupt model, Gemini may correctly choose not to call a tool (ambient noise), making the nudge incorrect. Known issue.
- **BLOCKING converse + immediate tool response** (`gemini.ts`): The `converse` tool is BLOCKING — Gemini freezes entirely (no audio, no text) when it calls the tool. The tool response is sent immediately as `{ result: "done" }` — no Claude text is sent back to the main session, eliminating context contamination. After unfreeze, Outer Gemini is free to hear new speech and call tools (including `stop` or another `converse`). A persistent TTS session (`tts-session.ts`) handles audio output independently.
