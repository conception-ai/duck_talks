...
- **TTS session architecture** (`tts-session.ts`): Ephemeral Gemini Live session opened per converse call. Fully self-contained — owns its own `GoogleGenAI` connection, `createSentenceBuffer`, and `createPlayer`. Each sentence-buffer flush is sent directly via `sendClientContent(turnComplete: true)` — no client-side queue. Non-obvious behaviors discovered experimentally:
  ...
  - **Multiple `turnComplete:true` during an active turn get merged** — Gemini treats concurrent sends as one big turn. This is the desired behavior: sentence-buffer flushes go directly to Gemini and get spoken continuously.
  ...
  - **`finish()` vs `close()`**: `finish()` flushes remaining buffer and lets audio drain naturally (closes on next `turnComplete`). `close()` is immediate teardown for interrupts — flushes player, closes session.
